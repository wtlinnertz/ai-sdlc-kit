# SAD — Embedded Reference Data Module
...
- Host application: Invokes the module's lookup interface in-process
...
This module ships as a library inside its single consuming deployable. Every interaction (lookup rules, data access, logging) is an internal component interaction within one deployment unit; per the SAD specification, internal component interactions are exempt and no cross-service contract is required.
